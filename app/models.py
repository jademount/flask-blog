...
class User(UserMixin, db.Model):
    ...
    def get_reset_password_token(self, expire_in=600):
        return jwt.encode({'reset_password':self.id, exp:time()+expire_in}, app.config['SECRET_KEY'], algorithms='HS256').decode('utf-8')

    @staticmethod
    def verify_reset_password_token(token):
        try:
            id=jwt.decode(token, app.config['SECRET_KEY'], app.config['SECRET_KEY'], algorithms='HS256')[reset_password]
        except:
            return
