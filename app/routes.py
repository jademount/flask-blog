...
def follow(username):
    ...
    db.session.commit()
    flash(_('You are following user:%(username)s!', username=username))
    return redirect(url_for('user',username=username))
...
def unfollow(username):
    ...
    db.session.commit()
    flash(_('You are unfollowing user:%(username)s.',username=username))
    return redirect(url_for('user',username=username))
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        flash(_('Check you email for the instructions to reset password'))
        return redirect(url_for('login'))
